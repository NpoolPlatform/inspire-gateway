    ...
    stage('Release docker image for development') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          set +e
          docker images | grep inspire-gateway | grep $branch
          rc=$?
          set -e
          if [ 0 -eq $rc ]; then
    ...
    stage('Deploy for development') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          sed -i "s/inspire-gateway:latest/inspire-gateway:$branch/g" cmd/inspire-gateway/k8s/02-inspire-gateway.yaml
          ...
          sed -i "s/replicas: 2/replicas: $REPLICAS_COUNT/g" cmd/inspire-gateway/k8s/02-inspire-gateway.yaml
    ...
    stage('Deploy for testing') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          sed -i "s/replicas: 2/replicas: $REPLICAS_COUNT/g" cmd/inspire-gateway/k8s/02-inspire-gateway.yaml
          sed -i "s/imagePullPolicy: Always/imagePullPolicy: IfNotPresent/g" cmd/inspire-gateway/k8s/02-inspire-gateway.yaml
          make deploy-to-k8s-cluster
        '''.stripIndent())
      }
    }
